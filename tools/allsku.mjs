...
const sel = all.filter(x=>/para\W*military\W*2|para\W*3/i.test(x.title)).map(x=>({title:x.title,handle:x.handle,published:x.published_at,skus:x.variants.map(v=>v.sku+' :: '+v.title)}));
fs.writeFileSync('site.json', JSON.stringify(sel,null,1));
